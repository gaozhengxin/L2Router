...
contract SushiswapV2Proxy {
    address private thefactory;
    address public admin;

    modifier onlyAdmin() {
        require(msg.sender == admin, "SushiswapV2Proxy: FORBIDDEN");
        _;
    }
    ...
    function setFactory(address _factory) public onlyAdmin returns (bool) {
        require(thefactory == address(0), "SushiswapV2Proxy: FORBIDDEN");
        require(_factory != address(0), "SushiswapV2Proxy: address(0x0)");
        thefactory = _factory;
        emit LogSetFactory(factory());
        return true;
    }

    event LogSetFactory(address indexed factory);
    ...
    function sortTokens(address tokenA, address tokenB)
        ...
        returns (address token0, address token1)
    {
        return SushiswapV2Library.sortTokens(tokenA, tokenB);
    }
    ...
    {
        return SushiswapV2Library.getReserves(factory(), tokenA, tokenB);
    }
