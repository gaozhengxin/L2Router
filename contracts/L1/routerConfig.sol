...
interface IConfigQuery {
    ...
    function getAllMultichainTokens(string calldata tokenID)
        external
        view
        returns (MultichainToken[] memory);

    function getMultichainToken(string calldata tokenID, uint256 chainID)
        external
        view
        returns (address);
    ...
    function getTokenConfig(string calldata tokenID, uint256 chainID)
        external
        view
        returns (TokenConfig memory);

    function getUserTokenConfig(string calldata tokenID, uint256 chainID)
        external
        view
        returns (TokenConfig memory);

    function getSwapConfig(string calldata tokenID, uint256 toChainID)
        external
        view
        returns (SwapConfig memory);

    function getCustomConfig(uint256 chainID, string calldata key)
        external
        view
        returns (string memory);

    function getMPCPubkey(address mpcAddress)
        external
        view
        returns (string memory);
    ...
    modifier checkChainconfig(ChainConfig memory config) {
        ...
        require(
            bytes(config.BlockChain).length > 0 &&
                bytes(config.BlockChain).length <= 128,
            "wrong BlockChain length"
        );
        _;
    }
    ...
    modifier checkSwapConfig(SwapConfig memory config) {
        ...
        require(
            config.MaximumSwap >= config.MinimumSwap,
            "MaximumSwap < MinimumSwap"
        );
        require(
            config.MaximumSwapFee >= config.MinimumSwapFee,
            "MaximumSwapFee < MinimumSwapFee"
        );
        require(
            config.MinimumSwap >= config.MinimumSwapFee,
            "MinimumSwap < MinimumSwapFee"
        );
        require(
            config.SwapFeeRatePerMillion < 1000000,
            "SwapFeeRatePerMillion >= 1000000"
        );
        require(
            config.SwapFeeRatePerMillion > 0 || config.MinimumSwapFee == 0,
            "wrong MinimumSwapFee"
        );
        _;
    }
}
...
contract RouterConfig is IConfigQuery {
    ...
    mapping(bytes32 => mapping(uint256 => TokenConfig))
        private _userTokenConfig; // key is tokenID,chainID
    ...
    function getMultichainToken(string calldata tokenID, uint256 chainID)
        ...
        returns (address)
    {
        MultichainToken[] storage _mcTokens = _allMultichainTokens[
    ...
    function isChainIDExist(uint256 chainID)
        public
        view
        override
        returns (bool)
    ...
    function setChainConfig(uint256 chainID, ChainConfig calldata config)
        external
        onlyOwner
        checkChainconfig(config)
        returns (bool)
    {
    ...
    ) internal checkSwapConfig(config) returns (bool) {
        require(tokenID != bytes32(0), "empty tokenID");
    ...
    function setTokenConfig(
        ...
        return
            _setTokenConfig(stringToBytes32(tokenID), chainID, config, false);
    }

    function setUserTokenConfig(
        string calldata tokenID,
        uint256 chainID,
        TokenConfig calldata config
    ) external checkTokenConfig(config) returns (bool) {
        return _setTokenConfig(stringToBytes32(tokenID), chainID, config, true);
    }

    function pickUserTokenConfig(string calldata tokenID, uint256 chainID)
        external
        onlyOwner
        returns (bool)
    {
        ...
        return
            _setTokenConfig(
                bsTokenID,
                chainID,
                _userTokenConfig[bsTokenID][chainID],
                false
            );
    }
    ...
    function setMPCPubkey(address addr, string calldata pubkey)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function addTokenID(string calldata tokenID)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function removeTokenID(string calldata tokenID)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function removeAllMultichainTokens(string calldata tokenID)
        external
        onlyOwner
    {
        bytes32 bsTokenID = stringToBytes32(tokenID);
    ...
    function removeMultichainToken(string calldata tokenID, uint256 chainID)
        external
        onlyOwner
        returns (bool)
    {
